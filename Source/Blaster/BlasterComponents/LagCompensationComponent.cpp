...
{
	// 캐릭터에게 붙인 HitBox들을 매 프레임 저장하고 만약 HitScanWeapon에서 이 함수를 호출한다면
	// 저장했던 프레임들과 시간으로 비교하여 어디에 있는지를 알아내고, 그 후 Hit했는지 체크
...
{
	// HitCharacter의 BoxFrame을 저장하는 함수
...
{
	// HitBox의 Transform값을 Pacakage에 있는 값으로 적용
	if (HitCharacter == nullptr) return;
	for (auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
	{
		if (HitBoxPair.Value != nullptr)
...
{
	// 매 프레임마다 모든 BoxComponent를 저장하고 특정 시간(MaxRecordTime)을 넘으면
	// 오래된 것을 삭제하고 최근거로 갱신
		if (Character == nullptr || !Character->HasAuthority()) return;
    	if (FrameHistory.Num() <= 1)
    	{
                                                              ...
                                                              ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if (World)
	{
		// Head만 먼저 충돌설정해서 LineTrace에 걸리는지 체크하여 헤드샷 확인
		...
		else // 헤드샷이 아니라면, 다른 부위도 충돌설정해서 LineTrace로 체크
			{
			for (auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
			{
				if (HitBoxPair.Value != nullptr)
