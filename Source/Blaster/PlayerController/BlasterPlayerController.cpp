...
#include "BlasterTypes/Announcementa.h"
...
void ABlasterPlayerController::OnPossess(APawn* InPawn)
{
	...
	if(BlasterCharacter)
	{
		...
		BlasterCharacter->SetDisableGamePlay(false);
	}
}
	...
	if(CharacterOverlay == nullptr)
	{
		if(BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if(CharacterOverlay)
			{
				...
				if (bInitializeWeaponAmmo) SetHUDCarriedAmmo(HUDWeaponAmmo);
...
void ABlasterPlayerController::CheckPing(float DeltaTime)
{
	...
	if(HighPingRunningTime > CheckPingFrequency)
	{
		...
		if(PlayerState)
		{
			// GetPing함수는 압축된 상태라 정확하지 않음 4로 나눠 압축하기 때문에 4를 곱해줘야함
...
void ABlasterPlayerController::ServerRequestServerTime_Implementation(float TimeOfClientRequest)
{
	...
	// 클라이언트에게 받은 시간을 다시 클라이언트에게 서버 시간과 같이 전달
...
{
	// 현재 클라이언트 시간 - (서버->클라이언트 시간 +클라이언트->서버 시간 + 이전 클라이언트 시간)으로
	// 보내는데 걸리는 시간, 즉 RoundTripTime을 얻을 수 있다.
